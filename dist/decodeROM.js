...
exports.LAST_FACTORY_WAVE = exports.FIRST_FACTORY_WAVE = exports.ROM_OFFSET = exports.HEAD_SIZE = exports.SLOT_SIZE = exports.TABLE_START_BYTES = void 0;
...
exports.TABLE_START_BYTES = 47296;
exports.SLOT_SIZE = 192;
exports.HEAD_SIZE = 128;
exports.ROM_OFFSET = -32;
exports.FIRST_FACTORY_WAVE = 32;
exports.LAST_FACTORY_WAVE = 125;
 ...
 */
function validateWaveRange(startIndex, count) {
    ...
    if (startIndex < exports.FIRST_FACTORY_WAVE) {
        throw new Error(`Waveform indices before ${exports.FIRST_FACTORY_WAVE} are not valid for the ROM-backed factory set; got startIndex=${startIndex}.`);
    }
    const endIndexExclusive = startIndex + count;
    if (endIndexExclusive > exports.LAST_FACTORY_WAVE + 1) {
        throw new Error(`Waveform range must end at or before ${exports.LAST_FACTORY_WAVE}; got startIndex=${startIndex}, count=${count}.`);
    }
}
...
function availableRomSlots(mem) {
    const usable = mem.length - exports.TABLE_START_BYTES;
    if (usable < 0)
        return 0;
    return Math.floor(usable / exports.SLOT_SIZE);
}
/**
 * Decode a single waveform by its index.
 * Returns null if the corresponding ROM slot falls outside the available data.
 */
function decodeWave(mem, waveformIndex) {
    if (waveformIndex < exports.FIRST_FACTORY_WAVE || waveformIndex > exports.LAST_FACTORY_WAVE) {
        return null;
    }
    const romIndex = waveformIndex + exports.ROM_OFFSET;
    if (romIndex < 0)
        return null;
    const slotStart = exports.TABLE_START_BYTES + romIndex * exports.SLOT_SIZE;
    const slotEnd = slotStart + exports.SLOT_SIZE;
    if (slotStart < 0 || slotEnd > mem.length)
        return null;
    const headStart = slotStart;
    const headEnd = slotStart + exports.HEAD_SIZE;
    const tailStart = headEnd;
    const tailEnd = slotEnd;
    const nSamples = exports.HEAD_SIZE; // one head byte per sample

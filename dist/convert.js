...
function parseArgs(argv) {
    ...
    if (opts.startIndex < decodeROM_1.FIRST_FACTORY_WAVE || opts.startIndex + opts.count > decodeROM_1.LAST_FACTORY_WAVE + 1) {
...
function main() {
    ...
    if (opts.vswavePath) {
        ...
        const mem = (0, decodeROM_1.interleaveRom)(msb, lsb);
        const nSlots = (0, decodeROM_1.availableRomSlots)(mem);
        ...
        waves = (0, decodeROM_1.decodeWaveRange)(mem, opts.startIndex, opts.count);
        provenance =
            `Source: ${path.basename(opts.msbPath)} / ${path.basename(opts.lsbPath)}\n` +
                `Generated: ${new Date().toISOString()}\n` +
                `ROM layout: fixed slot=${decodeROM_1.SLOT_SIZE} bytes, head=${decodeROM_1.HEAD_SIZE} bytes 8-bit, ` +
                `tail=${decodeROM_1.SLOT_SIZE - decodeROM_1.HEAD_SIZE} bytes packed nibbles, ` +
                `tableStart=${decodeROM_1.TABLE_START_BYTES} (absolute address of waveform 32), romOffset=${decodeROM_1.ROM_OFFSET}.\n` +
                `Format: sample12 = int8(headByte)*16 + fineNibble; fineNibble packed two-per-byte ` +
                `(high nibble = earlier sample) in the ${decodeROM_1.SLOT_SIZE - decodeROM_1.HEAD_SIZE} bytes ` +
                `following the head.`;
    }
    ...
    if (opts.verbose) {
        for (const w of waves) {
            console.log(`  waveform ${w.waveformIndex} <- ${opts.vswavePath ? "waveform index" : `ROM table offset ${w.romIndex}`}`);
        }
    }

...
function firstWaveOrderingLooksSwapped(mem: Uint8Array): boolean {
  const firstWave = decodeWave(mem, FIRST_FACTORY_WAVE);
  if (!firstWave) {
...
export function decodeWaveRangeFromRomPair(
  msb: ArrayLike<number>,
  lsb: ArrayLike<number>,
  startIndex: number,
  count: number
): RomDecodeResult {
